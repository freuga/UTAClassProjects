...
void initUart0()
{
    ...
    UART0_CTL_R = UART_CTL_TXE | UART_CTL_RXE | UART_CTL_UARTEN | UART_CTL_EOT;    // enable TX, RX, and module
    UART0_IM_R = 0;
    UART0_ICR_R = 0xFFFFFFFF;
    NVIC_EN0_R &= ~(1<<5);
...
// Parses string and saves into 2D array words up to the limit of max_words
uint8_t parseStr(char* str, char** words, uint8_t strSz, uint8_t maxWords)
{
    uint8_t wordCount = 0;
    uint8_t index = 0;

    //checking if first index starts the first word
    if(str[0] != ' ' && str[0] != '\0')
    {
        words[0] = &str[0];
        wordCount++;
    ...
    for(index = index; index < strSz; index++)
    {
        ...
        {
            str[index] = '\0';
            //if the next position is a character, save in list
            if(str[index+1] != '\0' && str[index+1] != ' ' && str[index+1] != '.')
            {
                words[wordCount] = &str[index+1];
                wordCount++;
            }
        }
    }
